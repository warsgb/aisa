...
- **客户360报告功能**
  - 后端：新增 Customer360 模块和 EJS 模板系统
  - 后端：新增 AI 销售洞察生成功能，基于客户数据智能分析销售机会
  - 后端：新增客户跟进记录（Followup）功能
  - 后端：新增快查（Kuaicha）代理服务支持，通过环境变量配置
  - 后端：新增数据库迁移脚本运行工具（migration:run）
  - 前端：客户卡片新增"生成360"和"查看360"按钮
  - 前端：移动端首页客户轮播卡片新增360快捷入口
  - 前端：移动端添加新客户后自动执行AI填充
  - 前端：优化移动端按钮显示，小屏幕隐藏文字仅显示图标
  - 配置：nginx 新增 /customer360/ 和 /reports/ 反向代理配置
  - 配置：新增 USE_KUAICHA_FOR_AUTO_FILL 和 USE_KUAICHA_PROXY 环境变量
  - 配置：新增 VITE_BACKEND_URL 环境变量用于静态资源访问

### Changed

- **客户360报告模板升级**
  - 新增 AI 销售洞察卡片（可折叠）
  - 新增 3 个 Tab 页面：客户档案、技能文档、跟进记录
  - 使用响应式设计，支持桌面和移动端
  - 集成 marked.js 支持 Markdown 渲染
...
- **教育行业客户研究技能 (education-customer-research)**
  - 新增完整教育行业客户研究技能，支持高等教育、K12、职业教育等客户类型
  - 包含11个真实教育行业客户案例（清华大学、深圳大学、海淀教委等）
  - 创建教育行业专属痛点库、数据源指南、案例库
  - 模板文件：关键摘要洞察、基础学校情况表、学术业务情况表、部门业务逻辑表、决策链条表、招投标信息表、分校区情况表

- **共享框架更新**
  - `industry-pain-points.md`: 扩展教育行业痛点（高等教育、K12、职业教育、通用核心痛点）
  - `decision-chain-framework.md`: 新增教育行业决策链（高等教育、K12、职业教育）及关键角色特点
  - `requirements-discovery-framework.md`: 新增教育行业SPIN提问话术（高等教育、K12、职业教育场景）

### Fixed

- **LTC流程配置**
  - 修复节点绑定技能功能缺失问题，现在可以点击技能卡片添加/删除绑定
  - 权限从OWNER扩展到OWNER和ADMIN

- **客户背景资料编辑**
  - 修复编辑时显示错误客户数据的bug（竞态条件）
  - 修复modal中数据未同步更新的问题
