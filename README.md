...
如果在服务器上拉取最新代码后无法正常使用，请检查以下配置：

#### 1. 修改前端 API 地址

代码中的 `.env` 文件默认配置为本地开发地址（`localhost:3001`），在服务器上部署时需要修改：
...
# 改为服务器实际 IP（假设服务器 IP 是 192.168.1.100）：
VITE_API_URL=http://192.168.1.100:3001
VITE_WS_URL=http://192.168.1.100:3001
```

```bash
# 方法2：创建 .env.local 文件覆盖（推荐，不会被 git 提交）
cd /opt/aisa
cat > .env.local << 'EOF'
VITE_API_URL=http://你的服务器IP:3001
VITE_WS_URL=http://你的服务器IP:3001
EOF
```
...
```bash
# 如果使用 PM2
pm2 restart aisa-frontend

# 如果使用 start-frontend-dev.sh
pkill -f "vite"
cd /opt/aisa
...
访问前端页面，打开浏览器开发者工具（F12），查看 Console 是否还有 `401 Unauthorized` 或 `ERR_CONNECTION_REFUSED` 错误。
